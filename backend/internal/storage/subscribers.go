...
const SubscribersTableName = "job_subscribers"
...
func (db DB) PostSubscribers(query v1beta.SubscribersPostQuery) (*v1beta.Subscribers, error) {
	...
	err := db.c.Transaction(func(tx *gorm.DB) error {
		...
		err := tx.Table(SubscribersTableName).Where("email = ?", query.Email).First(subscriber).Error
		if err == nil {
			return nil
		}
		...
		err = tx.Table(SubscribersTableName).Create(subscriber).Error
		if err != nil {
			return err
		}
