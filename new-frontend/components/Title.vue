<script setup lang="ts">
  ...
  const { title, subTitle, fontSize, onlyTitle,
    content, myStyle, header } = defineProps({
		title: {type: String, required: true},
    subTitle: {type: String, required: true},
    fontSize: {type: String, required: false, default: ''},
    onlyTitle: {type: String, required: false, default: ''},
    content: {type: String, required: false, default: ''},
    myStyle: {type: String, required: false, default: ''},
    header: {type: String, required: false, default: ''}
	});

  const isOpen: Ref<boolean> = ref(false);
  const opened: Ref<any[]> = ref([]);
  const tooltips: Ref<any[]> = ref([{id: 0, name: 'title'}]);
